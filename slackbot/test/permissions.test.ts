...
describe('checkToolPermission', () => {
  ...
  describe('admin user permissions', () => {
    it('should allow admin to read routes', () => {
      const result = checkToolPermission(adminUser, 'list_routes', 'links.example.com');
      expect(result.allowed).toBe(true);
      expect(result.userLevel).toBe('admin');
    });

    it('should allow admin to create routes', () => {
      const result = checkToolPermission(adminUser, 'create_route', 'links.example.com');
      expect(result.allowed).toBe(true);
    });

    it('should allow admin to delete routes', () => {
      const result = checkToolPermission(adminUser, 'delete_route', 'links.example.com');
      expect(result.allowed).toBe(true);
    });
  });
  ...
  describe('edit user permissions', () => {
    it('should allow edit user to read routes', () => {
      const result = checkToolPermission(editUser, 'list_routes', 'links.example.com');
      expect(result.allowed).toBe(true);
    });

    it('should allow edit user to create routes', () => {
      const result = checkToolPermission(editUser, 'create_route', 'links.example.com');
      expect(result.allowed).toBe(true);
    });

    it('should NOT allow edit user to delete routes', () => {
      const result = checkToolPermission(editUser, 'delete_route', 'links.example.com');
  ...
  describe('read-only user permissions', () => {
    it('should allow read user to read routes', () => {
      const result = checkToolPermission(readOnlyUser, 'list_routes', 'links.example.com');
      expect(result.allowed).toBe(true);
    });

    it('should NOT allow read user to create routes', () => {
      const result = checkToolPermission(readOnlyUser, 'create_route', 'links.example.com');
      expect(result.allowed).toBe(false);
    ...
    it('should NOT allow read user to delete routes', () => {
      const result = checkToolPermission(readOnlyUser, 'delete_route', 'links.example.com');
      expect(result.allowed).toBe(false);
    });
  });
  ...
  describe('domain-specific permissions', () => {
    ...
    it('should allow admin access to multiple domains', () => {
      const result1 = checkToolPermission(adminUser, 'delete_route', 'links.example.com');
  ...
  describe('null permissions (no user record)', () => {
    it('should deny all access when permissions are null', () => {
      const result = checkToolPermission(null, 'list_routes', 'links.example.com');
      expect(result.allowed).toBe(false);
      expect(result.userLevel).toBe('none');
  ...
  describe('unknown tools', () => {
    it('should default to admin permission for unknown tools', () => {
      const result = checkToolPermission(editUser, 'unknown_tool', 'links.example.com');
      expect(result.allowed).toBe(false);
      expect(result.requiredLevel).toBe('admin');
    });
  });
...
describe('getAccessibleDomains', () => {
  ...
  it('should return all domains with read access by default', () => {
    ...
    expect(domains).toContain('links.example.com');
    expect(domains).toContain('example.com');
    expect(domains).toContain('secondary.example.net');
  });
  ...
  it('should return only domains with edit access when specified', () => {
    ...
    expect(domains).toContain('links.example.com');
    expect(domains).toContain('example.com');
    expect(domains).not.toContain('secondary.example.net');
  });
