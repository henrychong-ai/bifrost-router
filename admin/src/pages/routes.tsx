...
import { Plus, MoreHorizontal, Pencil, Trash2, Power, PowerOff, ExternalLink, Search, X, Info } from 'lucide-react';
...
function RouteTypeBadge({ type }: { type: Route['type'] }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-tiny font-gilroy font-medium border ${styles[type]}`}>
      {type}
    </span>
  );
}
...
function RouteForm(props: RouteFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const baseData = {
      ...
      statusCode: formData.type === 'redirect' ? (formData.statusCode as 301 | 302 | 307 | 308) : undefined,
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        >
          ...
          <SelectContent>
            <SelectItem value="redirect" className="font-gilroy">Redirect</SelectItem>
            <SelectItem value="proxy" className="font-gilroy">Proxy</SelectItem>
            <SelectItem value="r2" className="font-gilroy">R2</SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-1">
          <Label htmlFor="target" className="font-gilroy font-medium text-charcoal-700">Target</Label>
          {formData.type === 'r2' && (
            <Tooltip>
              ...
              <TooltipContent side="top" className="max-w-xs">
                <p>Enter the file path within the selected R2 bucket. This is the object key — just the filename or folder path, not a URL.</p>
                <p className="mt-1 text-muted-foreground">Examples:<br/>• <code>bio.pdf</code> — file in bucket root<br/>• <code>images/header.jpg</code> — file in subfolder</p>
              </TooltipContent>
            </Tooltip>
          )}
        </div>
      ...
      {formData.type === 'redirect' && (
        <>
          <div className="space-y-2">
            <Label htmlFor="statusCode" className="font-gilroy font-medium text-charcoal-700">Status Code</Label>
            <Select
              value={String(formData.statusCode)}
              onValueChange={(value) => setFormData({ ...formData, statusCode: Number(value) as 301 | 302 | 307 | 308 })}
            >
              <SelectTrigger className="font-gilroy">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="301" className="font-gilroy">301 (Permanent)</SelectItem>
                <SelectItem value="302" className="font-gilroy">302 (Temporary)</SelectItem>
                <SelectItem value="307" className="font-gilroy">307 (Temporary, preserve method)</SelectItem>
                <SelectItem value="308" className="font-gilroy">308 (Permanent, preserve method)</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="flex items-center justify-between rounded-lg border border-charcoal-200 p-3">
            <div className="space-y-0.5">
              <Label htmlFor="preserveQuery" className="font-gilroy font-medium text-charcoal-700">Preserve Query String</Label>
              <p className="text-tiny text-muted-foreground font-gilroy">Pass query parameters to the target URL</p>
            </div>
            <Switch
              id="preserveQuery"
          ...
          <div className="flex items-center justify-between rounded-lg border border-charcoal-200 p-3">
            <div className="space-y-0.5">
              <Label htmlFor="preservePath" className="font-gilroy font-medium text-charcoal-700">Preserve Path</Label>
              <p className="text-tiny text-muted-foreground font-gilroy">Append the URL path to the target (for wildcard routes)</p>
            </div>
            <Switch
              id="preservePath"
      ...
      {formData.type === 'r2' && (
        <>
          <div className="space-y-2">
            <Label htmlFor="bucket" className="font-gilroy font-medium text-charcoal-700">R2 Bucket</Label>
            <Select
            ...
            <p className="text-tiny text-muted-foreground font-gilroy">R2 bucket to serve files from</p>
          </div>

          <div className="flex items-center justify-between rounded-lg border border-charcoal-200 p-3">
            <div className="space-y-0.5">
              <Label htmlFor="forceDownload" className="font-gilroy font-medium text-charcoal-700">Force Download</Label>
              <p className="text-tiny text-muted-foreground font-gilroy">Force browser to download file instead of displaying inline</p>
            </div>
            <Switch
              id="forceDownload"
      ...
      {formData.type === 'proxy' && (
        <div className="space-y-2">
          <div className="flex items-center gap-1">
            <Label htmlFor="hostHeader" className="font-gilroy font-medium text-charcoal-700">Host Header (optional)</Label>
            <Tooltip>
              <TooltipTrigger asChild>
                <Info className="h-3.5 w-3.5 text-muted-foreground cursor-help" />
              </TooltipTrigger>
              <TooltipContent side="top" className="max-w-xs">
                Override the Host header sent to the origin. Use when proxying to CDNs like Webflow that use Host-based virtual hosting.
              </TooltipContent>
            </Tooltip>
          </div>
          <Input
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-1">
          <Label htmlFor="cacheControl" className="font-gilroy font-medium text-charcoal-700">Cache-Control (optional)</Label>
          <Tooltip>
            <TooltipTrigger asChild>
              <Info className="h-3.5 w-3.5 text-muted-foreground cursor-help" />
            </TooltipTrigger>
            <TooltipContent side="top" className="max-w-xs">
              Controls how long browsers remember this content. Leave blank unless you have a specific reason to change it.
            </TooltipContent>
          </Tooltip>
        </div>
        <Input
      ...
      <DialogFooter>
        ...
        <Button type="submit" disabled={isSubmitting} className="font-gilroy bg-blue-950 hover:bg-blue-900">
...
export function RoutesPage() {
  ...
  const hasActiveFilters = !!(filters.domain || filters.search || filters.type || filters.enabled !== undefined);
  ...
  const handleCreate = async (data: CreateRouteInput, domain: string) => {
    try {
      ...
      toast.error(`Failed to create route: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  };
  ...
  const handleUpdate = async (data: UpdateRouteInput, pathChanged: boolean, newPath?: string) => {
    ...
    try {
      ...
      toast.error(`Failed to update route: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await deleteRoute.mutateAsync({ path: deleteConfirmRoute.path, domain: deleteConfirmRoute.domain ?? filters.domain });
      toast.success('Route deleted successfully');
      setDeleteConfirmRoute(null);
    } catch (err) {
      toast.error(`Failed to delete route: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  };
  ...
  const handleToggle = async (route: Route) => {
    try {
      // Pass domain from route when in all-domains view to ensure correct mutation
      await toggleRoute.mutateAsync({ path: route.path, enabled: !route.enabled, domain: route.domain ?? filters.domain });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle className="font-gilroy font-semibold text-blue-950">Create Route</DialogTitle>
              <DialogDescription className="font-gilroy">Add a new route to the edge router.</DialogDescription>
            </DialogHeader>
            <RouteForm
              mode="create"
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <Select
            ...
            onValueChange={(value) =>
              setFilters({ ...filters, domain: value === 'all' ? undefined : value as typeof filters.domain })
            }
          >
            ...
            <SelectContent>
              <SelectItem value="all" className="font-gilroy">All domains</SelectItem>
              {SUPPORTED_DOMAINS.map((domain) => (
                <SelectItem key={domain} value={domain} className="font-gilroy font-mono text-small">
                  {domain}
                </SelectItem>
              ))}
            </SelectContent>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <Select
            ...
            onValueChange={(value) =>
              setFilters({ ...filters, type: value === 'all' ? undefined : (value as 'redirect' | 'proxy' | 'r2') })
            }
          >
            ...
            <SelectContent>
              <SelectItem value="all" className="font-gilroy">All types</SelectItem>
              <SelectItem value="redirect" className="font-gilroy">Redirect</SelectItem>
              <SelectItem value="proxy" className="font-gilroy">Proxy</SelectItem>
              <SelectItem value="r2" className="font-gilroy">R2</SelectItem>
            </SelectContent>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="all" className="font-gilroy">All</SelectItem>
              <SelectItem value="active" className="font-gilroy">Active</SelectItem>
              <SelectItem value="disabled" className="font-gilroy">Disabled</SelectItem>
            </SelectContent>
          </Select>
        </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-charcoal-100 bg-muted/30">
                  {!filters.domain && (
                    <TableHead className="font-gilroy font-semibold text-charcoal-700">Domain</TableHead>
                  )}
                  <TableHead className="font-gilroy font-semibold text-charcoal-700">Path</TableHead>
                  <TableHead className="font-gilroy font-semibold text-charcoal-700">Type</TableHead>
                  <TableHead className="font-gilroy font-semibold text-charcoal-700">Target</TableHead>
                  <TableHead className="font-gilroy font-semibold text-charcoal-700">Status</TableHead>
              ...
              <TableBody>
                {filteredRoutes.map((route) => (
                  ...
                  >
                    {!filters.domain && (
                      <TableCell className="font-mono text-small text-charcoal-600">{route.domain || '-'}</TableCell>
                    )}
                    <TableCell className="font-mono text-small font-medium text-blue-600">{route.path}</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-tiny font-gilroy font-medium border ${
                        route.enabled !== false
                    ...
                    <TableCell onClick={(e) => e.stopPropagation()}>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onClick={() => setEditRoute(route)} className="font-gilroy">
                            <Pencil className="h-4 w-4 mr-2" />
                            Edit
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleToggle(route)} className="font-gilroy">
                            {route.enabled !== false ? (
                              <>
                ...
                {filteredRoutes.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={filters.domain ? 5 : 6} className="text-center text-muted-foreground font-gilroy">
                      {hasActiveFilters ? 'No routes match the current filters' : 'No routes configured'}
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
      ...
      <Dialog open={!!editRoute} onOpenChange={() => setEditRoute(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="font-gilroy font-semibold text-blue-950">Edit Route</DialogTitle>
            <DialogDescription className="font-gilroy">Update route configuration for <code className="font-mono text-blue-600">{editRoute?.path}</code></DialogDescription>
          </DialogHeader>
          {editRoute && (
            <RouteForm
              mode="edit"
      ...
      <Dialog open={!!deleteConfirmRoute} onOpenChange={() => setDeleteConfirmRoute(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="font-gilroy font-semibold text-blue-950">Delete Route</DialogTitle>
            <DialogDescription className="font-gilroy">
              Are you sure you want to delete the route <code className="font-mono text-blue-600">{deleteConfirmRoute?.path}</code>? This
              action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteConfirmRoute(null)} className="font-gilroy">
              Cancel
            </Button>
            <Button variant="destructive" onClick={handleDelete} disabled={deleteRoute.isPending} className="font-gilroy">
